...
def write_solution_csv(model, D, M, I,T):
    ...
    writer.writerow(["district","year construct","type","modern","HPModel","Year","QTY","totalhouses","percent of totalhouses","heatcapacity"])
    for i in I:
        for m in M:
            for t in range(T):
                if model.getVarByName(f'hp_type_{str(m)}_at_house_type_{str(i)}_in_month_{str(t)}').X !=0:
                    ...
                    row=[I[i]["district"],I[i]["year of construction"],I[i]["type of building"],I[i]["modernization status"],m,t,int(p),I[i]["quantity"],percent, I[i]["max_heat_demand_Patrick"]]
                    writer.writerow(row)  
    f.close() 
    return
